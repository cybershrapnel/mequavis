// meq-chat.js
//
// ✅ Local Ollama support (DeepSeek, etc) added.
// ✅ All chat-proxy fetch URLs are now configurable via CHAT_PROXY_URL.
// ✅ Provider parsing fixed so model names with ":" (e.g. deepseek-r1:8b) work.
//
// OPTIONAL: set these BEFORE this script loads (in your HTML):
//   <script>
//     window.OLLAMA_BASE_URL = "http://127.0.0.1:11434";
//     window.CHAT_PROXY_URL  = "http://127.0.0.1/chat-proxy/chat-proxy.php"; // if you run it locally
//   </script>
...
window.MeqChat = (function () {
  ...
  // ---------------------------------------------------------------------------
  // ✅ CONFIG: LOCAL OLLAMA + (OPTIONAL) LOCAL CHAT PROXY
  // ---------------------------------------------------------------------------
  const OLLAMA_BASE_URL = String(window.OLLAMA_BASE_URL || "http://127.0.0.1:11434").replace(/\/+$/, "");
  const CHAT_PROXY_URL  = String(window.CHAT_PROXY_URL  || "https://xtdevelopment.net/chat-proxy/chat-proxy.php");
...
#chatSessionPanel #viewArchiveBtn {
  ...
  // MODEL HANDLING
  // ---------------------------------------------------------------------------
  // ✅ FIXED: split on FIRST ":" only so model can contain ":" (deepseek-r1:8b)
  function getCurrentModel() {
    if (!modelSelect) {
      return { provider: "ollama", model: "deepseek-r1:8b" };
    }

    const raw = String(modelSelect.value || "ollama:deepseek-r1:8b");
    const i = raw.indexOf(":");
    if (i === -1) return { provider: raw || "ollama", model: "deepseek-r1:8b" };

    return {
      provider: raw.slice(0, i) || "ollama",
      model: raw.slice(i + 1) || "deepseek-r1:8b"
    };
  }

  // ---------------------------------------------------------------------------
  // ✅ LOCAL OLLAMA CHAT (NO PHP PROXY)
  // ---------------------------------------------------------------------------
  async function callOllamaChat(model, messages) {
    try {
      ...
      console.log("callOllamaChat payload:", payload);
      ...
      if (!res.ok) {
        ...
        return `[Ollama error ${res.status}]: ${txt}`;
      }

      const data = await res.json();
      const reply = data && data.message && typeof data.message.content === "string"
        ? data.message.content
        : "";

      return reply && reply.trim().length ? reply : "[Empty reply from Ollama]";
    } catch (err) {
      return "[Ollama network error: " + err.message + "]";
    }
  }
  ...
  async function send(userText, opts) {
    ...
    // ✅ Route to local Ollama when provider === "ollama"
    const reply = (provider === "ollama")
      ? await callOllamaChat(model, state.messages)
      : await callPhpProxy(provider, model, state.messages);
    ...
    else if (provider === "ollama") label = `Ollama (${model})`;
    ...
    panel.innerHTML = `
      ...
      <div id="chatInfoContent">
        <div style="font-size:11px; color:#0ff;">
          <p><img src="ncz.png"></p>
          <p>MEQUA News:</p>
          <p>Available models have been updated and all should be working now.</p>
<p>You can run your own local models via Ollama but you must expose Ollama to allow mequavis.com to access it. run this in command prompt prior to starting Ollama (for windows). setx OLLAMA_ORIGINS "https://mequavis.com"</p>
<p>No local data is sent to mequavis.com, your browser just needs CORS access to the data locally. I may change that later though to log local chats too. maybe...
